    ...
    reader.onload = (e) => {
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const [name, email, availabilityStr] = line
            .split(",")
            .map((s) => s.trim());
...
export const generateGroups = (participants, numberOfGroups) => {
  const groups = [];
...
export const generateRoundRobinSchedule = (groups, startDate, availability) => {
  const matches = [];
  let matchId = 1;
...
const findCommonAvailability = (avail1, avail2, date) => {
