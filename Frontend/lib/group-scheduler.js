...
export const findBestTimeSlot = (
  ...
  const commonDays = player2Availability.filter((slot) =>
    player1Days.has(slot.day),
  );
...
const formatDateFromDay = (day, baseDate) => {
  const dayMap = {
    Lunes: 1,
    Monday: 1,
    Martes: 2,
  ...
  groups.forEach((group) => {
    ...
    for (let i = 0; i < players.length; i++) {
      for (let j = i + 1; j < players.length; j++) {
        ...
        const avail1 =
          playerAvailability.get(player1) || getDefaultAvailability();
        const avail2 =
          playerAvailability.get(player2) || getDefaultAvailability();
